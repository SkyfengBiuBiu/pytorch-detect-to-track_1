...
# =============================================================================
            ...
            D_e = torch.index_select(_D, 1, q_e)[action-1]
            D_s = torch.index_select(_D, 1, q_s)[action-1]

            scores = ((D_e-D_s)/(dt.float()+1e-6)).view(-1,1)
            label = torch.cuda.FloatTensor(ts.size(0),1).fill_(1) * action
# =============================================================================
#             number=_D[int(_p[-1]), int(_q[-1])]
# =============================================================================
            number=_D[int(_p[-1])-1, int(_q[-1])]
	...
	for cls_ix in range(1,num_classes): # skip background class
	    ...
	    class_paths = self.all_paths[cls_ix]
            ...
            for i_pth in range(num_paths):
                M = class_paths['all_scores'][i_pth].clone()[:, 1:]
                print(M)# softmax across classes (exclude bkg)
                pred_path, time, D = self.dpEM_max(M)
                print('pred_path',pred_path,' time',time,'D',D)
...
class VideoDataset(data.Dataset):
    ...
    def _extract_frames(self, v_path):
        ...
        tmp_dir = os.path.join('/home/fengy/Documents/pytorch-detect-to-track_1/output_videos', os.path.basename(v_path))
