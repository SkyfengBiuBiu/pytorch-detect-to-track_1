...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul  4 15:37:37 2019
...
# =============================================================================
	...
	for cls_ix in range(1,num_classes): # skip background class
	    ...
	    class_paths = self.all_paths[cls_ix]
            if class_paths is None: 
                print("No path for person")# skip classes with no paths
	        continue
            ...
            for i_pth in range(num_paths):
                ...
                if Ts.numel()==0 or Scores.numel()==0:
                    continue
	...
	lp_count = self.get_path_count(live_paths_scores)
        ...
        if len(path_order_score.size())!=0:
            _, inds = torch.sort(path_order_score, dim=-1, descending=True)
