    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes):
        ...
        for i_leg in range(num_legs):
            ...
            if self.training:
                ...
                rois[i_leg], leg_rois_label, \
                        leg_rois_target, leg_rois_inside_ws, leg_rois_outside_ws = leg_roi_data
                leg_rois_label = Variable(leg_rois_label.view(-1).long(), volatile=True)
                leg_rois_target = Variable(leg_rois_target.view(-1, leg_rois_target.size(2)), volatile=True)
                leg_rois_inside_ws = Variable(leg_rois_inside_ws.view(-1, leg_rois_inside_ws.size(2)), volatile=True)
                leg_rois_outside_ws = Variable(leg_rois_outside_ws.view(-1, leg_rois_outside_ws.size(2)), volatile=True)
                rois_label.append(leg_rois_label)
        ...
        if self.training:
            ...
            tracking_rois, tracking_rois_label, \
                    tracking_rois_target, tracking_rois_inside_ws, tracking_rois_outside_ws = tracking_roi_data
            tracking_rois_label = Variable(tracking_rois_label.view(-1).long(), volatile=True)
            tracking_rois_target = Variable(tracking_rois_target.view(-1, tracking_rois_target.size(2)), volatile=True)
            tracking_rois_inside_ws = Variable(tracking_rois_inside_ws.view(-1, tracking_rois_inside_ws.size(2)), volatile=True)
            tracking_rois_outside_ws = Variable(tracking_rois_outside_ws.view(-1, tracking_rois_outside_ws.size(2)), volatile=True)
            tracking_rois = Variable(tracking_rois, volatile=True)
