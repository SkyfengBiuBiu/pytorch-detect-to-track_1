...
def convert_frames_to_video(video_dataset, fps):
    ...
    pathIn=output_dir+'/'
    pathOut=os.path.join(output_dir, 'video.avi')
...
# 
  ...
  if args.net == 'res101':
    ...
    RFCN = resnet(imagenet_vid_classes, 50, pretrained=False, class_agnostic=args.class_agnostic)
  elif args.net == 'res18':
    RFCN = resnet(imagenet_vid_classes, 18, pretrained=False, class_agnostic=args.class_agnostic)
  else:
  ...
  for ivid in range(len(video_dataset)):
      ...
      while True:
          thresh=thresh-0.1
          paths = vid_post_proc.class_paths(path_score_thresh=thresh)
          print("thresh",thresh)
          if vid_post_proc.flag:
              visualize_with_paths(video_dataset, vid_post_proc)
              fps = 25.0
              convert_frames_to_video(video_dataset,fps)
              break
          elif thresh<=0.0:
              print("No object had been detected")
              break
